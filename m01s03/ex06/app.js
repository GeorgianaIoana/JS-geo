var companyWebsite = 'www.pixellab.ro';
var companyName = 'The JavaScripts SRL';
var companyRegistration = 'J40/11111/2017';
var companyCode = '12345678';
var companyAddress =
  'Strada Scripturilor, Numarul 94, Etaj 1, Sector 1, Bucuresti';
var brandName = 'Pixellab';

console.log(`
  Acest website ${companyWebsite} este administrat de catre
  ${companyName},
  cu sediul social in ${companyAddress},
  inregistrata la Registrul Comertului sub nr.
  ${companyRegistration},
  cod unic de inregistrare ${companyCode} („${brandName}”).
  Operatorul datelor cu caracter personal este
  ${companyName}, prin brand-ul „${brandName}”, cu
  punctul de lucru la adresa: ${companyAddress} ;
  In continuare, ${companyName} va fi numit „${brandName}”.`);
